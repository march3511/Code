...
class registrar {  
	...
	function ingresar() {
		...
		$meter = 0;
		$cod_ciudad = strip_tags($_POST['cod_ciudad']);
		$cod_dept = strip_tags($_POST['cod_dept']);
		$nombre = strip_tags($_POST['nombre']);


		$consulta = "SELECT * FROM ciudad WHERE cod_ciudad = '$cod_ciudad'";

		$result = mysql_query($consulta);

		while ($row = mysql_fetch_array($result))
		{
			if ($row['cod_ciudad'] == $cod_ciudad) {
				$meter=1;
			}
		}
		...
		if ($meter == 1) {
			//echo "<script type=\"text/javascript\"> alert(\"!!modificar\");</script>";
			$modificar = "UPDATE ciudad SET cod_dept = '$cod_dept',nombre = '$nombre' WHERE cod_ciudad = '$cod_ciudad'";
			...
			$ingresar = "INSERT INTO ciudad(cod_ciudad, cod_dept, nombre) VALUES ('$cod_ciudad','$cod_dept','$nombre')";
...
<body>
	<div id="page-wrapper">
		<div class="container-fluid">
			<div class="row">
				<div class="col-lg-12">
					<h1 class="page-header">Ingreso Ciudades</h1>
						...
						<div class="col-lg-6">
							<form role="form" method="post" action="Ingresar.php">
								<div class="form-group">
									...
									<input class="form-control" id="cod_ciudad" name="cod_ciudad">
								...
								<div class="form-group">
									...
									<select class="form-control" name="cod_dept" id="cod_dept" >
										<option></option>
										<?php
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){   
			$( "#cod_ciudad" ).autocomplete({
				source: "../../plantilla/ajax/buscar_ciudad.php",
				minLength: 1
			});

			$("#cod_ciudad").focusout(function(){
				$.ajax({
					url:'../../plantilla/ajax/ciudad.php',
					type:'POST',
					dataType:'json',
					data:{ matricula:$('#cod_ciudad').val()}
				}).done(function(respuesta){
					var json = eval(respuesta);
							//alert(json);

							document.getElementById("nombre").value = json[1];
							document.getElementById("cod_dept").value = json[2];
						});
			});                         
		});
	</script> 
