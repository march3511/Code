...
{	
	...
	$.ajax({
		...
		type: "POST",
		dataType: "json",
		url: "../../plantilla/ajax/ajax_compania.php?action=listar",
		success: function(data){
			...
			if(data.length > 0){
				...
				html += '<td>'+item.cod_compañia+'</td>'
				html += '<td>'+item.nombre+'</td>'
				html += '<td>'+item.tipo+'</td>'
				html += '<td><a href="#" id="editar" class="editar"  onclick="editar('+item.cod_compañia+')">  <span class="glyphicon glyphicon-pencil"></span> </a>	<a href="#" onclick="eliminar('+item.cod_compañia+')">  <span class="glyphicon glyphicon-trash"></span> </a></td>'
...
function eliminar(cod_compañia) {
	//alert(cod_compañia);

	$.ajax({
		type:"POST",
		url: "../../plantilla/ajax/eliminar_compania.php",
		data: "id="+cod_compañia
	}).done(function(msg){
		//alert(msg);
		if(msg == "ok"){
			alert("Eliminado con Exito");
			//window.locationf="../../consulta_reserva.php";
			$(location).attr('href', '../../contenido/Compania/Consultar.php');
		}else{alert("Ha ocurrido un Error");}

	});
}

function editar(cod_compañia){
	alert(cod_compañia);
}