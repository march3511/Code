...
{	
	...
	$.ajax({
		...
		type: "POST",
		dataType: "json",
		url: "../../plantilla/ajax/ajax_departamento.php?action=listar",
		success: function(data){
			...
			if(data.length > 0){
				...
				html += '<td>'+item.cod_dpto+'</td>'
				html += '<td>'+item.nombre+'</td>'
				html += '<td><a href="#" id="editar" class="editar"  onclick="editar('+item.cod_dpto+')">  <span class="glyphicon glyphicon-pencil"></span> </a>	<a href="#" onclick="eliminar('+item.cod_dpto+')">  <span class="glyphicon glyphicon-trash"></span> </a></td>'
...
function eliminar(cod_dpto) {
	//alert(cedula);

	$.ajax({
		type:"POST",
		url: "../../plantilla/ajax/eliminar_departamento.php",
		data: "id="+cod_dpto
	}).done(function(msg){
		//alert(msg);
		if(msg == "ok"){
			...
			$(location).attr('href', '../../contenido/Departamento/Consultar.php');
		}else{alert("Ha ocurrido un Error");}

	});
}

function editar(cod_dpto){
	alert(cod_dpto);
}