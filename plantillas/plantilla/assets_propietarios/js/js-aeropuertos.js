...
{	
	...
	$.ajax({
		...
		type: "POST",
		dataType: "json",
		url: "../../plantilla/ajax/ajax_aeropuertos.php?action=listar",
		success: function(data){
			...
			if(data.length > 0){
				...
				html += '<td>'+item.cod_aeropuerto+'</td>'
				html += '<td>'+item.nombre+'</td>'
				html += '<td>'+item.cod_ciudad+'</td>'
				html += '<td><a href="#" id="editar" class="editar"  onclick="editar('+item.cod_aeropuerto+')">  <span class="glyphicon glyphicon-pencil"></span> </a>	<a href="#" onclick="eliminar('+item.cod_aeropuerto+')">  <span class="glyphicon glyphicon-trash"></span> </a></td>'
...
function eliminar(cod_aeropuerto) {
	//alert(cedula);

	$.ajax({
		type:"POST",
		url: "../../plantilla/ajax/eliminar_aeropuertos.php",
		data: "id="+cod_aeropuerto
	}).done(function(msg){
		//alert(msg);
		if(msg == "ok"){
			alert("Eliminado con Exito");
			//window.locationf="../../consulta_reserva.php";
			$(location).attr('href', '../../contenido/Aeropuertos/Consultar.php');
		}else{alert("Ha ocurrido un Error");}

	});
}

function editar(cod_aeropuerto){
	alert(cod_aeropuerto);
}