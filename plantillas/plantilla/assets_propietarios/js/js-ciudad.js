...
{	
	...
	$.ajax({
		...
		type: "POST",
		dataType: "json",
		url: "../../plantilla/ajax/ajax_ciudad.php?action=listar",
		success: function(data){
			...
			if(data.length > 0){
				...
				html += '<td>'+item.nombre_dept+'</td>'
				html += '<td><a href="#" id="editar" class="editar"  onclick="editar('+item.cod_ciudad+')">  <span class="glyphicon glyphicon-pencil"></span> </a>	<a href="#" onclick="eliminar('+item.cod_ciudad+')">  <span class="glyphicon glyphicon-trash"></span> </a></td>'
...
function eliminar(cedula) {
	...
	$.ajax({
		type:"POST",
		url: "../../plantilla/ajax/eliminar_ciudad.php",
		data: "id="+cedula
	}).done(function(msg){
		//alert(msg);
		if(msg == "ok"){
			...
			$(location).attr('href', '../../contenido/Ciudad/Consultar.php');
