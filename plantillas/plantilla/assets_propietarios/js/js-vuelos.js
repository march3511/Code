...
{	
	...
	$.ajax({
		...
		type: "POST",
		dataType: "json",
		url: "../../plantilla/ajax/ajax_vuelos.php?action=listar",
		success: function(data){
			...
			if(data.length > 0){
				...
				html += '<td>'+item.hora_origen+'</td>'
				html += '<td>'+item.valor+'</td>'
				html += '<td><a href="#" id="editar" class="editar"  onclick="editar('+item.cod_vuelo+')">  <span class="glyphicon glyphicon-pencil"></span> </a>	<a href="#" onclick="eliminar('+item.cod_vuelo+')">  <span class="glyphicon glyphicon-trash"></span> </a></td>'
				html += '</tr></a>';
						//html += '</form>';													
					});						
			}
...
function eliminar(cod_vuelo) {
	//alert(cedula);

	$.ajax({
		type:"POST",
		url: "../../plantilla/ajax/eliminar_vuelos.php",
		data: "id="+cod_vuelo
	}).done(function(msg){
		//alert(msg);
		if(msg == "ok"){
			...
			$(location).attr('href', '../../contenido/Vuelos/Consultar.php');
		}else{alert("Ha ocurrido un Error");}

	});
}

function editar(cod_vuelo){
	alert(cod_vuelo);
}